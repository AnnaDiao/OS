...
打开 sign.c 文件，可知
1.磁盘主引导扇区只有512字节
2.磁盘最后两个字节为0x55、0xAA

* * *
###练习2
1. 在 terminal 下打开 /home/moocos/moocos/ucore_lab/labcodes/lab1/bin 路径，输入：
	```qemu -S -s -hda ucore.img -monitor stdio```
启动 qemu；
2. 再打开一个terminal 执行gdb，输入：
	```target remote 127.0.0.1:1234```
设定与 qemu 模拟器链接的 1234 端口号
	```
...
=> 0x7c02:	xor    %eax,%eax
   ...
   0x7c08:	mov    %eax,%ss
	```
8. 查看下面20条指令，对比```boot/bootasm.S``` 和 ```obj/bootblock.asm ```:
...
9.  bootload的主函数从 7cdc 开始，随意设置另一个断点 ```b*0x7cf0 -> continue```:
	
	```
	Breakpoint 2, 0x00007cf0 in ?? ()
(gdb) x/4i $pc
=> 0x7cf0:	jne    0x7cdf
   0x7cf2:	cmpl   $0x464c457f,0x10000
   0x7cfc:	jne    0x7d68
